...
namespace Novius\OnlineMediaFiles\Package;

/**
 * ExceptionInterface
 */
interface Vimeo_Api_ExceptionInterface {}

/**
 * Vimeo_Api_UploadException class for failure to upload to the server.
 */
class Vimeo_Api_UploadException extends \Exception implements Vimeo_Api_ExceptionInterface {}
...
class Vimeo_Api
{
    ...
    public function upload ($file_path, $upgrade_to_1080 = false, $machine_id = null)
    {
        ...
        if (!is_file($file_path)) {
            throw new Vimeo_Api_UploadException('Unable to locate file to upload.');
        }
    ...
    private function perform_upload($file_path, $ticket)
    {
        if ($ticket['status'] != 201) {
            throw new Vimeo_Api_UploadException('Unable to get an upload ticket.');
        }
        ...
        if ($status != 201) {
            throw new Vimeo_Api_UploadException('Error completing the upload.');
        }
    ...
    public function uploadImage ($pictures_uri, $file_path, $activate = false) {
        ...
        if ($pictures_response['status'] != 201) {
            throw new Vimeo_Api_UploadException('Unable to request an upload url from vimeo');
        }
        ...
        if (!$response) {
            ...
            throw new Vimeo_Api_UploadException($error);
        }
        curl_close($curl);

        if ($curl_info['http_code'] != 200) {
            throw new Vimeo_Api_UploadException($response);
        }
