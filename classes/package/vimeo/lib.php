...
class Package_Vimeo_Lib_APIException extends \Exception {}

class Package_Vimeo_Lib
{
    ...
    private function _request($method, $call_params = array(), $request_method = 'GET', $url = self::API_REST_URL, $cache = true, $use_auth_header = true)
    {
        ...
        if (!empty($method)) {
            ...
            if ($response->stat == 'ok') {
                ...
                throw new Package_Vimeo_Lib_APIException($response->err->msg, $response->err->code);
            }
     ...
     * Enable the cache.
     *
     * @param string $type The type of cache to use (Package_Vimeo_Lib::CACHE_FILE is built in)
    ...
    public function upload($file_path, $use_multiple_chunks = false, $chunk_temp_dir = '.', $size = 2097152, $replace_id = null)
    {
        ...
        if ($quota->user->upload_space->free < $file_size) {
            throw new Package_Vimeo_Lib_APIException('The file is larger than the user\'s remaining quota.', 707);
        }
        ...
        if ($file_size > $rsp->ticket->max_file_size) {
            throw new Package_Vimeo_Lib_APIException('File exceeds maximum allowed size.', 710);
        }
        ...
        if ($complete->stat == 'ok') {
            ...
            throw new Package_Vimeo_Lib_APIException($complete->err->msg, $complete->err->code);
        }
    }
    ...
    {
        if (is_array($input)) {
            return array_map(array('Package_Vimeo_Lib', 'url_encode_rfc3986'), $input);
        }
