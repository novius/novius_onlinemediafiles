...
class Permission
{
    ...
    {
        return \Nos\User\Permission::atMost('novius_onlinemediafiles::media', '1_draft_only', '2_full_access');
    }
    ...
    {
        return \Nos\User\Permission::exists('novius_onlinemediafiles::folder', 'no');
    }
    ...
    public static function isMediaInRestrictedFolder($media)
    {
        ...
        return !$media->folder || $media->folder->onfo_id == 1 ? false : static::isFolderRestricted($media->folder);
    }
    ...
    public static function isFolderRestricted($folder)
    {
        ...
        do {
            if (in_array($folder->onfo_id, $restricted_folder_ids)) {
    ...
    public static function getRestrictedFoldersIds()
    {
        ...
        $full_access = \Nos\User\Permission::check('novius_onlinemediafiles::restrict_folders', '_', true);
        if ($full_access) {
            return array();
        }
        return \Nos\User\Permission::listPermissionCategories('novius_onlinemediafiles::restrict_folders');
    }
    ...
    public static function getRestrictedFolders()
    {
        ...
        return Model_Folder::find('all', array(
            'where' => array(
                array('onfo_id', 'IN', $restricted_folder_ids),
            ),
        ));
    }
